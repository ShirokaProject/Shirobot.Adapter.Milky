...
- ShiroBot SDK `0.7.0-rc3`：适配器与 ShiroBot 宿主之间的接口契约版本，不代表 Milky 协议版本。
...
当相邻目录存在 `../ShiroBot/ShiroBot.SDK/ShiroBot.SDK.csproj` 时，默认使用本地 `ProjectReference`。否则使用正式依赖：

```xml
<PackageReference Include="ShiroBot.SDK" Version="0.7.0-rc3" />
```

发布或验证 NuGet 包时应显式传入 `-p:UseLocalShiroBotSdk=false`。
...
测试覆盖 Markdown 消息段、群解散事件、群通知多态、Webhook 错误隔离与取消、API 错误 envelope、有额外字段的新响应、请求 null 忽略、版本策略、multicast 事件等待，以及未知事件、消息段和枚举的容错行为。
