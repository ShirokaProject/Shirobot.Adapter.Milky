...
public class EventService : IEventService
{
    ...
    internal async Task OnEventReceivedAsync(Event e)
    {
        ...
        if (MilkyMapper.ToBotEvent(e) is not { } botEvent) return;

        foreach (Func<BotEvent, Task> subscriber in EventReceived.GetInvocationList())
            await subscriber(botEvent).ConfigureAwait(false);
    }
}
